...
<script setup lang="ts">
  ...
  const color = reactive({
    primary: localCache.getCache('theme-primary') || '#409eff',
    success: localCache.getCache('theme-success') || '#67c23a',
    warning: localCache.getCache('theme-warning') || '#e6a23c',
    danger: localCache.getCache('theme-danger') || '#f56c6c',
    info: localCache.getCache('theme-info') || '#909399',
  ...
  const resetHeader = () => {
    localCache.removeCache('header-bg-color');
    localCache.removeCache('header-text-color');
  };
  const resetSidebar = () => {
    localCache.removeCache('sidebar-bg-color');
    localCache.removeCache('sidebar-text-color');
  };
  const system = [
    {
      name: '默认',
